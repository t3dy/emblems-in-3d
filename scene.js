...
import { buildPapercraftSpace } from "./papercards.js?v=4";

// ===========================================================================
// scene.js — generate a 3D diorama for any Atalanta emblem from its catalogued
// data (palette, alchemical stage, visual_elements), place props from the prop
// library, attach the lab-process tour, and let the tour open a purpose-built
// specialized laboratory when the base lab can't host the operation.
// ===========================================================================
...
// ---- plain ground (used only by the specialized-lab interior) ----
...
// ---- toggle: hide every popped figure card, leaving only the flat backdrop
// plate at the rear — i.e. "show me the plain print" vs. the exploded pop-up.
let plateOnly = false;
function togglePlate() {
  plateOnly = !plateOnly;
  anim.interactables.forEach((c) => { c.visible = !plateOnly; });
  return !plateOnly;
}

let buildId = 0; // guards the async cutout loads against a stale/superseded build

async function buildEmblemScene() {
  clearWorld();
  const myBuild = ++buildId;
  const stage = (data.stage || "").toUpperCase();

  // mood by stage — the void behind the tunnel's open end
  const moodBg = { NIGREDO: 0x14110b, ALBEDO: 0x2a2c2e, CITRINITAS: 0x2a2412, RUBEDO: 0x24140e }[stage] || 0x1a160e;
  scene.background = new THREE.Color(moodBg);
  scene.fog = new THREE.Fog(moodBg, 20, 40);
  hemi.intensity = stage === "NIGREDO" ? 0.55 : stage === "ALBEDO" ? 1.05 : 0.85;
  sun.intensity = stage === "ALBEDO" ? 1.3 : 1.0;

  // ── the walk-through tunnel-book: every figure is the plate's own real
  // cutout (EmblemPrintShop extraction, EmblemPapercraft's depth manifest),
  // mounted as a curled paper card at real 3D depth — not a synthetic prop.
  const space = await buildPapercraftSpace({ THREE, world, anim }, n);
  if (myBuild !== buildId) return; // superseded by a newer navigation/build

  spawnPos = space.spawn; walkBounds = space.bounds;
  anim.interactables = space.interactables;
  settingLabelEl.textContent = `${space.interactables.length} paper cutout${space.interactables.length === 1 ? "" : "s"}`;

  const accent = hexToColor(data.palette.glow, "#caa45a");
  const accLight = new THREE.PointLight(accent.getHex(), stage === "NIGREDO" ? 2.4 : 1.6, 30, 2);
  accLight.userData.base = accLight.intensity;
  accLight.position.set(0, 6, 0); world.add(accLight); anim.lums.push(accLight);
