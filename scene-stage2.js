...
import { makeSkyGradient, makeSoilTexture, makeGrassTexture, makeCobbleTexture, makeWoodFloorTexture, makeSandTexture, makeCheckerTexture } from "./textures.js?v=2";

// ===========================================================================
// scene-stage2.js — PRESERVED SNAPSHOT, not the live site (see
// docs/HISTORY.md, Stage 2): the last version before the perspective-
// armature rewrite. A flat 44×44 ground plane (textured per the emblem's
// `locations.js` setting — soil/grass/cobble/wood/sand), the emblem's
// catalogued `visual_elements` tags turned into primitive props scattered
// across it by rough category, `bespoke.js` hand-composed staging on top,
// the toon+ink-edge woodcut post-process — and the actual engraving hung on
// a wall as a framed picture. The textures, ink shader, setting labels, and
// bespoke staging all read correctly here; what's wrong is spatial: it's a
// flat plane with a poster standing on it, not the room the emblem depicts.
// That's exactly what the perspective-armature rewrite (`scene-legacy.js`,
// Stage 3) fixed, before Stage 4 replaced the props themselves with real
// cutouts. Walk mode, music, "operate," and the scholarship panel were
// already in place here and survived every later rewrite unchanged.
// ===========================================================================
...
// ---- ground (textured per setting) + the engraving framed on the back wall ----
const FLOOR_TEX = { soil: makeSoilTexture, grass: makeGrassTexture, cobble: makeCobbleTexture, wood: makeWoodFloorTexture, sand: makeSandTexture, stone: makeCobbleTexture };
function ground(color, floorKey) {
  const map = floorKey && FLOOR_TEX[floorKey] ? FLOOR_TEX[floorKey]() : makeCheckerTexture(color);
  const g = new THREE.Mesh(new THREE.PlaneGeometry(44, 44), toon({ color: 0xffffff, map }));
  g.rotation.x = -Math.PI / 2; g.receiveShadow = true; world.add(g);
}
function backdropPlate(img) {
  const tex = new THREE.TextureLoader().load(img); tex.colorSpace = THREE.SRGBColorSpace;
  const frame = new THREE.Mesh(new THREE.BoxGeometry(7.2, 6.2, 0.3), toon({ color: 0x3a2c1a }));
  frame.position.set(0, 3.6, -10); world.add(frame);
  const pic = new THREE.Mesh(new THREE.PlaneGeometry(6.6, 5.6), new THREE.MeshBasicMaterial({ map: tex }));
  pic.position.set(0, 3.6, -9.82); world.add(pic);
}
function togglePlate() { return true; } // no separate toggle existed yet at this stage
...
function buildEmblemScene() {
  ...
  ground(settingDef.ground, settingDef.floor);
  backdropPlate(data.img);
  settingLabelEl.textContent = setting[0].toUpperCase() + setting.slice(1);
  spawnPos = { pos: new THREE.Vector3(0, 3.2, 12), look: new THREE.Vector3(0, 1.4, -2) };
  walkBounds = { minX: -19, maxX: 19, minZ: -19, maxZ: 19 };

  const accLight = new THREE.PointLight(accent.getHex(), stage === "NIGREDO" ? 3 : 2, 24, 2);
  accLight.userData.base = accLight.intensity;
  accLight.position.set(0, 3, 1); world.add(accLight); anim.lums.push(accLight);

  const span = (i, total, w) => (total <= 1 ? 0 : (i / (total - 1) - 0.5) * w);
  const byZone = {};
  ...
  Object.entries(byZone).forEach(([zone, tags]) => {
    tags.forEach((t, i) => {
      ...
      let x = span(i, tags.length, 9), y = 0, z = 0;
      if (zone === "sky") { y = 6; z = -6; }
      else if (zone === "scenery") { z = -4 - (i % 2) * 1.5; x = span(i, tags.length, 13); }
      else if (zone === "creature") { z = -1.5; x = span(i, tags.length, 8); }
      else if (zone === "actor") { z = 1.5; x = span(i, tags.length, 6); p.rotation.y = -x * 0.06; }
      else { z = -0.5; x = span(i, tags.length, 5); } // central
      place(p, x, y, z);
  ...
  // the location backdrop — extra props for the emblem's setting (farm,
  // laboratory, kitchen, …), placed at the far edge with a per-emblem seed
  // so plates sharing a setting don't render as identical carbon copies
  const seeded = (i) => { const x = Math.sin(n * 12.9898 + i * 78.233) * 43758.5453; return x - Math.floor(x); };
  settingDef.backdrop.forEach((b, i) => {
    const p = buildProp(ctx, b.tag);
    const jitterScale = (b.scale || 1) * (0.85 + seeded(i) * 0.4);
    p.scale.setScalar(jitterScale);
    p.rotation.y = seeded(i + 10) * Math.PI * 2;
    const jx = b.x + (seeded(i + 20) - 0.5) * 2.4;
    const jz = b.z + (seeded(i + 30) - 0.5) * 1.6;
    place(p, jx, 0, jz);
    if (p.userData.flame) anim.flames.push(p);
  });

  if (!byZone.actor) place(makeFigure(ctx, {}), 0, 0, 2);

  const bespokeStage = BESPOKE[String(n)];
  if (bespokeStage) bespokeStage({ THREE, ctx, place, buildProp, makeFigure, world, anim, data });

  camera.position.set(0, 3.2, 12); controls.target.set(0, 1.4, -2); controls.update();
  titleEl.textContent = `Emblem ${data.roman} — ${data.mottoEn}`;
  document.getElementById("s-env").style.display = id === "af-08" ? "" : "none";
}
